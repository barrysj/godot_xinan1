...
- 版本默认折叠。摘要显示对象名、版本、root、日期、首图、静态图片数、动作数，以及标签化的选用、批准和校验状态；校验状态由文件与 `.tres` 引用实时计算，不写入 Manifest，也不等于 `selection` 或 `approval`。
...
```powershell
pwsh.exe -NoProfile -File .\tools\art\asset_manager\control\build-control.ps1
...
```powershell
pwsh.exe -NoProfile -File .\tools\art\asset_manager\run-server.ps1
