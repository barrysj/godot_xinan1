"""Build Chalk Spirit candidate 005 from three generated RGB animation sheets.
...
ACTION_TIMING = {"windup": 0.20, "recovery": 0.25, "impact_ratio": 0.625}
COMBAT_UNIFORM_SCALE = 1.30
...
def normalize_combat_atlas(
    rgba: np.ndarray,
    records: list[dict],
    rows: int,
    cols: int,
) -> tuple[np.ndarray, list[dict]]:
    """Uniformly scale 003 combat poses around the shared logical foot anchor."""
    source = Image.fromarray(rgba, "RGBA")
    atlas = Image.new("RGBA", (CANVAS * cols, CANVAS * rows), (0, 0, 0, 0))
    scaled_size = round(CANVAS * COMBAT_UNIFORM_SCALE)
    crop_left = round(CANVAS / 2 * COMBAT_UNIFORM_SCALE - CANVAS / 2)
    crop_top = round(BASELINE * COMBAT_UNIFORM_SCALE - BASELINE)
    normalized_records = []

    for index, record in enumerate(records):
        x, y, width, height = record["region"]
        pose = Image.new("RGBA", (CANVAS, CANVAS), (0, 0, 0, 0))
        pose.alpha_composite(
            source.crop((x, y, x + width, y + height)),
            (round(record["margin"][0]), round(record["margin"][1])),
        )
        scaled = pose.resize((scaled_size, scaled_size), Image.Resampling.LANCZOS)
        scaled = scaled.crop((crop_left, crop_top, crop_left + CANVAS, crop_top + CANVAS))
        alpha = np.asarray(scaled)[:, :, 3]
        ys, xs = np.nonzero(alpha >= 80)
        if len(xs) == 0:
            raise ValueError(f"Empty normalized combat pose {index}")
        left, right = int(xs.min()), int(xs.max()) + 1
        top, bottom = int(ys.min()), int(ys.max()) + 1
        edge_margin = [left, top, CANVAS - right, CANVAS - bottom]
        if min(edge_margin) < SAFE_MARGIN:
            raise ValueError(f"Unsafe normalized combat margin {index}: {edge_margin}")
        col, row = index % cols, index // cols
        atlas.alpha_composite(scaled, (col * CANVAS, row * CANVAS))
        normalized_records.append({
            "pose": f"combat:{index}",
            "region": [col * CANVAS, row * CANVAS, CANVAS, CANVAS],
            "margin": [0, 0, 0, 0],
            "content_size": [right - left, bottom - top],
            "cell_edge_margin": edge_margin,
            "source_region": record["region"],
            "source_content_size": record["content_size"],
            "source_baseline": BASELINE,
        })
    return np.asarray(atlas), normalized_records
...
def process_sheet(name: str, rows: int, cols: int) -> tuple[dict, list[dict]]:
    ...
    for index in range(rows * cols):
        ...
        records.append({
            ...
            "content_size": [right_raw - left_raw, bottom_raw - top_raw],
            "cell_edge_margin": edge_margin,
            "source_baseline": bottom_raw,
        })

    if name == "combat":
        rgba, records = normalize_combat_atlas(rgba, records, rows, cols)
        alpha = rgba[:, :, 3]
    ...
    summary = {
        ...
        "output_size": list(Image.open(output).size),
        "grid": [cols, rows],
    ...
    if name == "combat":
        summary["uniform_scale"] = [COMBAT_UNIFORM_SCALE, COMBAT_UNIFORM_SCALE]
        summary["transform_origin"] = [CANVAS / 2, BASELINE]
...
def godot_resource(all_records: dict[str, list[dict]]) -> str:
    ...
    lines.extend([
        ...
        f'display_size = Vector2({DISPLAY_SIZE}, {DISPLAY_SIZE})',
...
def action_frame_durations(frame_count: int) -> list[int]:
    """Invert BattleAnimation.sync_motion so GIF timing matches runtime phases."""
    impact = ACTION_TIMING["impact_ratio"]
    windup = ACTION_TIMING["windup"]
    recovery = ACTION_TIMING["recovery"]
    durations = []
    for index in range(frame_count):
        start = index / frame_count
        end = (index + 1) / frame_count
        if end <= impact:
            seconds = (end - start) / impact * windup
        elif start >= impact:
            seconds = (end - start) / (1 - impact) * recovery
        else:
            seconds = (impact - start) / impact * windup
            seconds += (end - impact) / (1 - impact) * recovery
        durations.append(max(20, round(seconds * 1000)))
    return durations
...
def write_gif_previews(all_records: dict[str, list[dict]]) -> None:
    ...
    idle_context = ["locomotion:2", "locomotion:3"]
    idle_return = ["locomotion:0", "locomotion:1"]
    for clip, (poses, speed, loop) in CLIPS.items():
        raw_frames = [render_pose(pose, images, record_lookup) for pose in poses]
        frame_ms = round(1000 / speed)
        raw_durations = [frame_ms] * len(raw_frames)
        if not loop:
            raw_durations[-1] = max(frame_ms, 350)
        save_gif(review / f"{clip}-frames.gif", raw_frames, raw_durations)

        if clip in ("attack", "cast"):
            transition_poses = idle_context + poses + idle_return
            transition_durations = [167, 167] + action_frame_durations(len(poses)) + [167, 500]
        elif clip == "hurt":
            transition_poses = idle_context + poses + idle_return
            transition_durations = [167, 167] + [frame_ms] * len(poses) + [167, 500]
        elif clip == "death":
            transition_poses = idle_context + poses
            transition_durations = [167, 167] + [frame_ms] * len(poses)
            transition_durations[-1] = 500
        else:
            transition_poses = poses
            transition_durations = [frame_ms] * len(poses)
        transition_frames = [
            render_pose(pose, images, record_lookup) for pose in transition_poses
        ]
        save_gif(review / f"{clip}-preview.gif", transition_frames, transition_durations)

    comparison_poses = ["locomotion:0", "combat:0", "combat:7", "combat:8", "combat:15"]
    comparison = Image.new("RGBA", (CANVAS * len(comparison_poses), CANVAS), (31, 55, 52, 255))
    for index, pose in enumerate(comparison_poses):
        comparison.alpha_composite(render_pose(pose, images, record_lookup), (index * CANVAS, 0))
    comparison.save(review / "cross-clip-scale.png")


def scale_consistency(all_records: dict[str, list[dict]]) -> dict:
    locomotion_height = median(record["content_size"][1] for record in all_records["locomotion"])
    combat_height = median(record["content_size"][1] for record in all_records["combat"])
    ratio = combat_height / locomotion_height
    if not 0.90 <= ratio <= 1.10:
        raise ValueError(f"Combat/locomotion silhouette scale ratio out of range: {ratio:.3f}")
    source_aspect = median(
        record["source_content_size"][0] / record["source_content_size"][1]
        for record in all_records["combat"]
    )
    output_aspect = median(
        record["content_size"][0] / record["content_size"][1]
        for record in all_records["combat"]
    )
    aspect_delta = abs(output_aspect - source_aspect)
    if aspect_delta > 0.02:
        raise ValueError(f"Combat aspect ratio drifted during uniform scaling: {aspect_delta:.3f}")
    return {
        "metric": "median opaque silhouette height",
        "locomotion_px": locomotion_height,
        "combat_px": combat_height,
        "combat_to_locomotion": round(ratio, 4),
        "accepted_range": [0.90, 1.10],
        "uniform_scale": [COMBAT_UNIFORM_SCALE, COMBAT_UNIFORM_SCALE],
        "source_aspect_median": round(source_aspect, 4),
        "output_aspect_median": round(output_aspect, 4),
        "aspect_delta": round(aspect_delta, 4),
        "visual_evidence": (HERE / "review" / "cross-clip-scale.png").relative_to(ROOT).as_posix(),
    }
...
def main() -> None:
    ...
    metadata = {
        ...
        "runtime_preview_timing": ACTION_TIMING,
    }
    (HERE / "battle_animation.tres").write_text(godot_resource(poses), encoding="utf-8")
    write_gif_previews(poses)
    metadata["scale_consistency"] = scale_consistency(poses)
    (HERE / "battle_animation.frames.json").write_text(
        json.dumps(metadata, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    print("candidate 005: 44 poses, 7 clips")
    print("scale", metadata["scale_consistency"])
