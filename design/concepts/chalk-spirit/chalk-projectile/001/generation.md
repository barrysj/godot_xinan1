...
状态：资产候选，待人工评审。当前仅用于独立动作预览，不登记 Manifest，也不替换正式角色资源。
...
`build_projectile.py` 读取 `projectile_source_rgba.png`，将 alpha 小于 8 的生成噪点清零，按非透明包围盒裁切，再保持原横纵比缩放到 192×96 透明画布。有效内容为 164×64，运行显示尺寸为 32×16；脚本同时重建候选 `.tres`、元数据和静态尺寸预览。
...
- 运行资源：`chalk_projectile.tres`，锚点 `[0.5, 0.5]`，尾迹间距 2 逻辑像素。
...
技术检查通过只表示透明度、尺寸、资源加载、方向旋转和回退路径正常，不代表资产已获批准。批准后再复制到 `assets/art/`、登记 Manifest 并绑定正式角色。
