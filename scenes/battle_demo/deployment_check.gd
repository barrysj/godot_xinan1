...
func run_checks() -> void:
	...
	check(panel.ghost.visible and not panel.accept_button.visible, "Hover previews without confirmation")
	...
	check(panel.candidate_slot == 0, "Clicked preview stays fixed when moving toward buttons")
	...
	await click(card_point(2))
	await click(slot_point(1))
	check(panel.ghost.visible and panel.accept_button.visible and panel.previews_unit(0), "Reserve card click previews occupied replacement")
	await click(panel.accept_button.position + Vector2(17,17))
	check(game.formation[1] == 2 and not game.formation.has(0), "Reserve card click replaces deployed ally after confirmation")
	await click(card_point(0))
	await click(slot_point(1))
	await click(panel.accept_button.position + Vector2(17,17))
	...
	check(panel.swap_ghost.visible and panel.accept_button.visible, "Deployed card click previews direct occupied swap")
	await click(panel.accept_button.position + Vector2(17,17))
	check(game.formation[1] == 1 and game.formation[3] == 0, "Deployed card click swaps occupied cells after confirmation")
	await click(card_point(0))
	await click(slot_point(1))
	await click(panel.accept_button.position + Vector2(17,17))
...
func check_deployed_actions() -> void:
	...
	await move(slot_point(3))
	check(panel.ghost.visible and panel.swap_ghost.visible and not panel.accept_button.visible, "Swap hover previews both destinations")
	await click(slot_point(3))
	await click(panel.cancel_button.position + Vector2(17,17))
	check(game.formation == original, "Cancel swap preserves both units")
	await click(slot_point(1))
	await click(panel.swap_button.position + Vector2(32,15))
	await click(slot_point(3))
	await click(panel.accept_button.position + Vector2(17,17))
	check(game.formation[1] == 1 and game.formation[3] == 0, "Click swap exchanges occupied cells atomically")
	var restored = game.Checkpoint.decode(game.progress.active_run)
	...
	await move(slot_point(1))
	check(panel.dragging and panel.swap_ghost.visible and not panel.accept_button.visible, "Dragging deployed unit skips action selection")
	await mouse(slot_point(1), false)
	...
	await move(card_point(0))
	check(panel.return_hint.visible and not panel.accept_button.visible, "Drag back shows withdraw hint")
...
func capture() -> void:
	for resolution in [Vector2i(1920,1080), Vector2i(2560,1440), Vector2i(1920,1200)]:
		...
		await click(card_point(0))
		await click(slot_point(4))
		await screenshot("card-swap-preview", resolution)
