extends Node2D
## Review harness for the approved hybrid cutout Bone2D and sequence-frame asset.
...
func _ready() -> void:
	...
	if "--chalk-spirit-hybrid-check" in args:
		call_deferred("_run_check")
	elif "--chalk-spirit-hybrid-capture" in args:
...
func _draw() -> void:
	...
	_text(Vector2(62, 78), "粉笔精灵混合动画 006", 30, INK)
	_text(Vector2(size.x - 500, 76), "正式 005 vs 混合方案 006 · 0.5×", 20, INK)
	...
	_text(Vector2(size.x * 0.61, 178), "资产 006 · 混合动画", 24)
...
func _run_check() -> void:
	...
	if rig.visible or not hybrid_sequence_sprite.visible:
		failures += 1
	print("CHALK_SPIRIT_HYBRID_CHECK actions=7 rig=6 death=approved-005 failures=", failures)
...
func _capture() -> void:
	...
	var attack_frames := "res://.godot/chalk-spirit-hybrid-frames"
	...
	var tour_frames := "res://.godot/chalk-spirit-hybrid-tour-frames"
	...
	for mode in MODES:
		for local_index in 16:
			...
			frame_index += 1
	print("CHALK_SPIRIT_HYBRID_CAPTURE actions=7 attack_frames=40 tour_frames=112 resolutions=3")
	get_tree().quit()
