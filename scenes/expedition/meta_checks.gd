...
func run_checks(hub) -> void:
	...
	hub._test_click(hub._home_rect("resume").get_center())
	...
	hub._to_base()
	hub._test_click(hub._home_rect("growth").get_center())
	verify(hub.screen == "growth", "growth entry")
	hub._test_click(Vector2(1100,657))
	hub._test_click(hub._home_rect("dispatch").get_center())
	verify(hub.screen == "dispatch", "dispatch entry")
	hub.screen = "base"
	hub._test_click(hub._home_rect("codex").get_center())
	verify(is_instance_valid(hub.codex_panel) and not hub.paused, "home codex entry")
	hub.codex_panel.close_guide()
	verify(hub.screen == "base" and not hub.paused, "home codex return")
	hub._test_click(hub._home_rect("new").get_center())
	verify(hub.paused and hub.pending_exit == "new_run", "home new expedition confirmation")
	hub._resume_battle()
	hub._test_click(hub._home_rect("dispatch").get_center())
