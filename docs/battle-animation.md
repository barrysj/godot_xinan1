...
粉笔精灵 006 验证了分层刚性骨骼：从已批准待机帧确定性切出主体、发射臂和两块浮游石，由 `Skeleton2D` / `Bone2D` 连续插值，粉尘和弹体保持独立。实际对比中，固定部件消除了跨帧主体宽高、横纵比和朝向漂移，动作幅度与时序也可不重绘直接调整；退场仍保留序列帧。完整记录与 GIF 见[粉笔精灵混合动画 006](art/tasks/chalk-spirit.md#骨骼序列帧混合方案-006)。
...
- 预览约束：继续使用统一 `run-motion-preview.ps1` 指定角色、动作和速度；独立混合动画 runner 只用于重建候选 GIF 与骨骼结构自检。
...
非像素风拆件嵌入像素战斗场景时，不得继承最近邻过滤：旋转与缩小会让采样点逐帧跳变，形成水波纹式边缘闪烁。骨骼表现根节点应显式使用线性 mipmap 过滤，拆件导入启用 mipmap；不要用像素对齐量化连续骨骼位置来掩盖问题。仅作为刚性图片枢轴、不参与 IK 或长度求解的 `Bone2D` 应关闭自动长度与角度计算并提供稳定的占位长度，避免叶骨骼警告。
