...
旧固定站桩与固定内容主路径。玩家价值：保留旧档兼容价值。

## 实现与规则入口

- 实现入口：[short_run.gd](../../game/run/short_run.gd)、[gameplay.gd](../../scenes/gameplay/gameplay.gd)
- 规则事实源：[录入](../content-authoring.md)
- 跨系统边界参见[集成约束](integration.md)；本文件不另存规则数值或美术审批记录。

## 验证与证据

- 已有证据：静态、E04、E14。编号和重跑入口见[共享验证目录](verification.md)。
- 以上是继承的验收记录，不表示本次文档整理重新验证。新增验证在此更新日期、代码基线、命令、结果与覆盖边界；共享检查变化同步目录。
- 程序通过不等于真人体验、目标设备或美术审批通过。

## 已知边界与未完成项

旧 STAGES／算法保留兼容；新内容进入 Resource；不要删除迁移依赖
