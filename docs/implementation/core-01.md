...
基地到 Boss 结算再出发。玩家价值：短局闭环。

## 实现与规则入口

- 实现入口：[meta_hub.gd](../../scenes/expedition/meta_hub.gd)、[expedition.gd](../../scenes/expedition/expedition.gd)
- 规则事实源：[校园](../campus-demo.md)
- 跨系统边界参见[集成约束](integration.md)；本文件不另存规则数值或美术审批记录。

## 验证与证据

- 已有证据：E04、E05。编号和重跑入口见[共享验证目录](verification.md)。
