...
当前版本桌面／浏览器／手机可用性。玩家价值：真实设备可玩。

## 实现与规则入口

- 实现入口：[export_presets.cfg](../../export_presets.cfg)
- 规则事实源：[发布](../web-publishing.md)
- 跨系统边界参见[集成约束](integration.md)；本文件不另存规则数值或美术审批记录。

## 验证与证据

- 已有证据：当前原生专项＋E18。编号和重跑入口见[共享验证目录](verification.md)。
- 以上是继承的验收记录，不表示本次文档整理重新验证。新增验证在此更新日期、代码基线、命令、结果与覆盖边界；共享检查变化同步目录。
- 程序通过不等于真人体验、目标设备或美术审批通过。

## 已知边界与未完成项

Windows 与本地桌面 Web 通过；844×390 可操作但不可读，真机触控未通过验收
