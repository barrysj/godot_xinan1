...
地图选地点、多人成队、离线派遣与领取。玩家价值：基地可操作循环。

## 实现与规则入口

- 实现入口：[location_dispatch_panel.gd](../../scenes/expedition/location_dispatch_panel.gd)、[campus_progress.gd](../../game/meta/campus_progress.gd)
- 规则事实源：[校园地图](../campus-map.md)
- 跨系统边界参见[集成约束](integration.md)；本文件不另存规则数值或美术审批记录。

## 验证与证据

- 已有证据：E05–E08。编号和重跑入口见[共享验证目录](verification.md)。
