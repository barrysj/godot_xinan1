...
- 接入回归：`MOTION_PREVIEW_CHECK failures=0`、`PRESENTATION_CHECK PASS failures=0`、`SKILL_VFX_CHECK failures=0`、`ANIMATION CHECK: PASS`。真实战斗检查显式验证表现场景实例化、骨骼动作处理及 death 回退；未再出现 `Bone2D` 自动骨长警告。资源台以标准 clip ID `attack` 识别远程 GIF，页面文件接口返回 `image/gif`；七动作巡演 GIF 只保留为任务评审证据，不伪装成单一动作。
