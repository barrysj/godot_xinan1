...
- 状态日期：2026-09-15；分支：main；功能基线：`7fe7529`。初始统筹基线为 `1ad601f`，M0 后续实现与文档提交为 `205784b` 至 `7fe7529`。
...
- 初始统筹及 M0 使用 Windows Godot 4.7.2 运行无头检查、图形集成检查和当前 Web release 包验证，见第 3 节。当前桌面浏览器已覆盖完整短局与刷新恢复；没有 Android／iOS 真机、线上部署、其他操作系统、陌生玩家局长与长期平衡证据。测试截图不替代人工美术批准。
- `.godot/audit-20260914-*.log` 为本机忽略缓存，不随提交流转；以下记录关键结果与可重跑入口，不把历史截图当作当前画面证据。
- M0 已于 `7fe7529` 完成工程验收：随机路线允许经真实失败—重配—重试完成，图鉴检查进入真实战斗，首战引导与后续阵位沿用接通，Web 检查点写后同步并完成桌面 release 包验证。人工试玩与真机边界仍按第 7 节登记，不因里程碑完成而自动视为通过。
- 原始 `docs/tmp/` 摘要继续保留且未跟踪，不纳入本轮正式状态提交。
...
| DEPLOY-01 | 部署 | 手牌点选／拖拽直接部署、换人、换位及撤回 | 已验证 | 快速调整阵容 | [deployment_panel.gd](../scenes/battle_demo/deployment_panel.gd) | [战斗](battle-demo.md) | E03、E04、E12 | 操作菜单可点空白收起；交换来源亮边；自身格、右键或 Esc 可取消；有效目标立即生效；首战空场、后续沿用；触控待验 |
